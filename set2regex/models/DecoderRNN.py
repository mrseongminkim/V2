...
class DecoderRNN(BaseRNN):
    ...
    # except sos, eos
    def __init__(self, vocab_size, max_len, hidden_size, n_layers=1, rnn_cell="LSTM", bidirectional=False, input_dropout_p=0, dropout_p=0, use_attention=False, attn_mode=False, sos_id=0, eos_id=1):
    ...
    def forward_step(self, input_var, hidden, encoder_outputs, function, embedding=None):
        batch_size, output_size = input_var.shape
        ...
        embedded = one_hot.view(batch_size, output_size, self.vocab_size).float()
        ...
        predicted_softmax = function(self.out(output.contiguous().view(-1, self.hidden_size)), dim=1).view(batch_size, self.output_size, -1)

        return predicted_softmax, hidden, attn

    def forward(self, inputs=None, embedding=None, encoder_hidden=None, encoder_outputs=None, function=F.log_softmax, teacher_forcing_ratio=0, masking=None):
        ret_dict = dict()
        if self.use_attention:
        ...
        decoder_hidden = self.hidden_out1(decoder_hidden)
        use_teacher_forcing = True if random.random() < teacher_forcing_ratio else False

        # step * batch * vocab_size
        decoder_outputs = []
        sequence_symbols = []
        ...
        if use_teacher_forcing:
            ...
            decoder_output, decoder_hidden, attn = self.forward_step(input_var=decoder_input, hidden=decoder_hidden, encoder_outputs=encoder_outputs, function=function)
            ...
            for di in range(max_length):
                ...
                decoder_output, decoder_hidden, step_attn = self.forward_step(input_var=decoder_input, hidden=decoder_hidden, encoder_outputs=encoder_outputs, function=function)
