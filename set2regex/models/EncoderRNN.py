...
class EncoderRNN(BaseRNN):
    def __init__(
        ...
        self.pool_linear = nn.Linear(hidden_size * 6, hidden_size * 2)
    ...
    def concat_pooling(self, hiddens):
        ...
        n_examples = hiddens.size(2)
        avg_pool = torch.sum(hiddens, dim=2) / n_examples
        max_pool = torch.max(hiddens, dim=2).values
        min_pool = torch.min(hiddens, dim=2).values

        pool = torch.cat((min_pool, avg_pool, max_pool), dim=-1)
        pool = self.pool_linear(pool)
        # 2, 512, 512
        return avg_pool
    ...
    def forward_example(self, pos):
        ...
        if self.set_transformer:
            set_hidden = self.rnn2(pos_set).squeeze(1).unsqueeze(0).repeat_interleave(2, dim=0)
            if type(self.rnn1) is nn.LSTM:
                set_cell = torch.zeros_like(set_hidden)

                pos_cell = self.concat_pooling(self._cat_directions(pos_hidden[1]))
                pos_hidden = self.concat_pooling(self._cat_directions(pos_hidden[0]))
                ...
                pos_hidden = self.concat_pooling(self._cat_directions(pos_hidden))
