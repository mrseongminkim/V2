...
from utils import set_seed, init_weights, count_parameters, train_fn, evaluate_fn, EarlyStopping, synthesize_regex
...
early_stop = EarlyStopping(patience=n_epochs, path=expt_dir)
...
if train:
    ...
    model.load_state_dict(torch.load("./saved_models/set2regex/practical/lstm/model.pt"))
    test_data_loader = get_data_loader(train_file_path, usage, pad_index, regex_max_length, 1, num_worker, False)
    # test_data_loader = get_data_loader("./data/practical_data/integrated/test_snort.csv", usage, pad_index, regex_max_length, 1, num_worker, False)
    for i, batch in enumerate(test_data_loader):
        pos = batch["pos"].to(device)
        neg = batch["neg"].to(device)
        regex = batch["regex"].to(device)

        x = synthesize_regex(pos, neg, model, vocab, device, regex_max_length)
        x = x[1:-1]
        result = "".join(x)

        regex = "".join(vocab.lookup_tokens(regex)[1:-1])
