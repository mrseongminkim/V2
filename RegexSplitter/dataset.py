...
def get_data_loader(file_path, usage, pad_index, batch_size, num_worker, shuffle, regex_max_length=None):
    def collate_fn(batch):
        if usage == "train":
            ...
            for data in batch:
                ...
                batch_label_ids.extend(data["label_ids"])
            batch_train_pos_ids = nn.utils.rnn.pad_sequence(batch_train_pos_ids, padding_value=pad_index)
            batch_label_ids = nn.utils.rnn.pad_sequence(batch_label_ids, padding_value=pad_index)
            batch = {
